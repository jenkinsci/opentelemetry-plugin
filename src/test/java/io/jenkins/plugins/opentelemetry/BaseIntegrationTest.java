...
public class BaseIntegrationTest {
    ...
    protected void assertNodeMetadata(Tree<SpanDataWrapper> spans, String jobName, boolean withNode) throws Exception {
        ...
        if (withNode) {
            MatcherAssert.assertThat(attributes.get(JenkinsAttributes.JENKINS_STEP_AGENT_LABEL), CoreMatchers.not(Matchers.emptyOrNullString()));
        } else {
            MatcherAssert.assertThat(attributes.get(JenkinsAttributes.JENKINS_STEP_AGENT_LABEL), CoreMatchers.is(Matchers.emptyOrNullString()));
        }
        MatcherAssert.assertThat(attributes.get(JenkinsAttributes.CI_PIPELINE_AGENT_NAME), CoreMatchers.not(Matchers.emptyOrNullString()));
        MatcherAssert.assertThat(attributes.get(JenkinsAttributes.CI_PIPELINE_AGENT_ID), Matchers.notNullValue());
    }
    ...
    protected void assertBuildStepMetadata(Tree<SpanDataWrapper> spans, String stepName, String pluginName) throws Exception {
        ...
        MatcherAssert.assertThat(attributes.get(JenkinsAttributes.JENKINS_STEP_PLUGIN_NAME), CoreMatchers.is(pluginName));
    ...
    public static class SpanDataWrapper {
        ...
        public String toString() {
            ...
            if (attributes.get(JenkinsAttributes.JENKINS_STEP_TYPE) != null) {
                result += ", function: " + attributes.get(JenkinsAttributes.JENKINS_STEP_TYPE);
            }
            if (attributes.get(JenkinsAttributes.JENKINS_STEP_NAME) != null) {
                result += ", name: " + attributes.get(JenkinsAttributes.JENKINS_STEP_NAME);
            }
            if (attributes.get(JenkinsAttributes.JENKINS_STEP_ID) != null) {
                result += ", node.id: " + attributes.get(JenkinsAttributes.JENKINS_STEP_ID);
            }
            return result;
        }
    }
