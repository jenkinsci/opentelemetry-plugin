...
public class StepResultTest extends BaseIntegrationTest {
    ...
    public void testSimplePipelineWithWithStepResults() throws Exception {
        ...
        checkChainOfSpans(spans, "unstable", "Stage: build", ExtendedJenkinsAttributes.AGENT_UI, "Phase: Run", rootSpanName);
        checkChainOfSpans(spans, "parallel-first", "Parallel branch: first", "Stage: parallel", ExtendedJenkinsAttributes.AGENT_UI, "Phase: Run", rootSpanName);
        checkChainOfSpans(spans, "parallel-second", "Parallel branch: second", "Stage: parallel", ExtendedJenkinsAttributes.AGENT_UI, "Phase: Run", rootSpanName);
        ...
        { // node span: 'ABORTED'  (because timeout stage is aborted)
            SpanData actualSpanData = spans.breadthFirstStream().filter(sdw -> ExtendedJenkinsAttributes.AGENT_UI.equals(sdw.spanData.getName())).findFirst().get().spanData;
            String actualStageResult = actualSpanData.getAttributes().get(ExtendedJenkinsAttributes.JENKINS_STEP_RESULT);
