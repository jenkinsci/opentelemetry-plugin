...
public class StepResultTest extends BaseIntegrationTest {
    ...
    public void testSimplePipelineWithWithStepResults() throws Exception {
        ...
        checkChainOfSpans(spans, "unstable", "Stage: build", JenkinsAttributes.AGENT_UI, "Phase: Run", rootSpanName);
        checkChainOfSpans(spans, "parallel-first", "Parallel branch: first", "Stage: parallel", JenkinsAttributes.AGENT_UI, "Phase: Run", rootSpanName);
        checkChainOfSpans(spans, "parallel-second", "Parallel branch: second", "Stage: parallel", JenkinsAttributes.AGENT_UI, "Phase: Run", rootSpanName);
        checkChainOfSpans(spans, "Stage: skipped", JenkinsAttributes.AGENT_UI, "Phase: Run", rootSpanName);
        checkChainOfSpans(spans, "Stage: timeout", JenkinsAttributes.AGENT_UI, "Phase: Run", rootSpanName);
        ...
        { // node span: 'ABORTED'  (because timeout stage is aborted)
            SpanData actualSpanData = spans.breadthFirstStream().filter(sdw -> JenkinsAttributes.AGENT_UI.equals(sdw.spanData.getName())).findFirst().get().spanData;
            String actualStageResult = actualSpanData.getAttributes().get(JenkinsAttributes.JENKINS_STEP_RESULT);
            MatcherAssert.assertThat(actualStageResult, CoreMatchers.is("ABORTED"));
        }

        { // node allocation span: 'SUCCESS'
            SpanData actualSpanData = spans.breadthFirstStream().filter(sdw -> JenkinsAttributes.AGENT_ALLOCATION_UI.equals(sdw.spanData.getName())).findFirst().get().spanData;
            String actualStageResult = actualSpanData.getAttributes().get(JenkinsAttributes.JENKINS_STEP_RESULT);
        ...
        { // parallel node 'first': 'FAILURE'
            ...
            String actualStepResult = actualSpanData.getAttributes().get(JenkinsAttributes.JENKINS_STEP_RESULT);
