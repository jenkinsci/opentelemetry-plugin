...
public class OpenTelemetryConfigurerComputerListener extends ComputerListener
    ...
    public void preOnline(Computer computer, Channel channel, FilePath root, TaskListener listener) {
        ...
        OpenTelemetryConfiguration openTelemetryConfiguration =
                jenkinsOpenTelemetryPluginConfiguration.toOpenTelemetryConfiguration();
    ...
    public void afterConfiguration(@NonNull ConfigProperties configProperties) {
        ...
        boolean otlpLogsEnabled = "otlp"
                .equals(configProperties.getString(
                        "otel.logs.exporter")); // pipeline logs export to OTLP endpoint activated
        boolean jenkinsAgentInstrumentationDisabled = "false"
        ...
        Arrays.stream(computers).forEach(computer -> {
            ...
            logger.log(
                    Level.FINE,
                    () -> "Evaluate computer.name: '" + computer.getName() + "', node: "
            ...
            if (node instanceof Jenkins) {
                ...
                logger.log(
                        Level.FINE,
                        () -> "Failure to update OpenTelemetry configuration for computer/build-agent '"
                                + computer.getName() + "' as its channel is null, probably offline");
            } else {
                configureAgentResults.add(configureOpenTelemetrySdkOnComputer(
                        computer, channel, otelSdkProperties, otelSdkResourceProperties));
            }
        });
    ...
    private Future<Object> configureOpenTelemetrySdkOnComputer(
        ...
        final Set<String> filteredResourceKeys = Set.of(
                ServiceAttributes.SERVICE_NAME.getKey(), ServiceIncubatingAttributes.SERVICE_INSTANCE_ID.getKey());
        buildAgentOtelSdkProperties = new HashMap<>(otelSdkProperties);
        buildAgentOtelSdkResourceProperties = new HashMap<>();
        otelSdkResourceProperties.entrySet().stream()
        ...
        callable = new OpenTelemetryConfigurerMasterToSlaveCallable(
                buildAgentOtelSdkProperties, buildAgentOtelSdkResourceProperties);

        try {
            logger.log(
                    Level.FINE,
                    () -> "Updating OpenTelemetry configuration for computer/build-agent '" + computer.getName()
                            + "'...");
