...
public abstract class AbstractMonitoringAction implements OtelMonitoringAction {
    ...
    public AbstractMonitoringAction(Span span, List<Scope> scopes) {
        this.spanAndScopes =
                new SpanAndScopes(span, scopes, Thread.currentThread().getName());
        this.traceId = span.getSpanContext().getTraceId();
        this.spanId = span.getSpanContext().getSpanId();
        this.spanName = span instanceof ReadWriteSpan
                ? ((ReadWriteSpan) span).getName()
                : null; // when tracer is no-op, span is NOT a ReadWriteSpan
        try (Scope scope = span.makeCurrent()) {
            Map<String, String> w3cTraceContext = new HashMap<>();
            W3CTraceContextPropagator.getInstance()
                    .inject(Context.current(), w3cTraceContext, (carrier, key, value) -> carrier.put(key, value));
        ...
        LOGGER.log(
                ...
                () -> "Span " + getSpanName()
                        + Optional.ofNullable(spanAndScopes)
                                .map(sas -> ", thread=" + sas.scopeStartThreadName + " opened " + sas.scopes.size()
                                        + " scopes")
    ...
    public void purgeSpanAndCloseAssociatedScopes() {
        LOGGER.log(
                Level.FINE,
                () -> "Purge span='" + spanName + "', spanId=" + spanId + ", traceId=" + traceId + ": "
                        + spanAndScopes);
        Optional.ofNullable(spanAndScopes)
    ...
    static class SpanAndScopes {
        ...
        public String toString() {
            return "SpanAndScopes{" + "span="
                    + span + ", scopes="
                    + scopes.size() + ", scopeStartThreadName='"
                    + scopeStartThreadName + '\'' + '}';
        }
    }
}
