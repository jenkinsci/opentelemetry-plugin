...
public class SpanAttributeStepExecution extends GeneralNonBlockingStepExecution {
    ...
    public SpanAttributeStepExecution(
            ...
            StepContext context,
            boolean setAttributesOnlyOnParent) {
        super(context);
        this.spanAttributes = spanAttributes;
        this.setOnChildren = setOnChildren;
    ...
    protected Void run() throws Exception {
        ...
        String currentSpanId =
                otelTraceService.getSpan(run, flowNode).getSpanContext().getSpanId();
        ...
        spanAttributes.forEach(spanAttribute -> {
            logger.log(
                    Level.FINE,
                    () -> "spanAttribute: run=\"" + run.getParent().getName() + "#" + run.getId() + "\", key="
                            + spanAttribute.getKey() + " value=\"" + spanAttribute.getValue() + "\" type="
                            + spanAttribute.getAttributeType());
            spanAttribute
                    .getTargetSpan()
                    .setAttribute(spanAttribute.getAttributeKey(), spanAttribute.getConvertedValue());
        });
        if (setOnChildren) {
            spanAttributes.forEach(spanAttribute -> {
                switch (spanAttribute.getTarget()) {
                    ...
                    // Ideally we would set the attribute on all child spans that are still in progress and log a
                    // warning
                    // for closed child spans. (We cannot change attributes on a span that is closed, as it might
                    // already have been sent out.)
                    // Child spans created after the execution of withSpanAttributes will all have the attribute set
                    // correctly.
                    case PIPELINE_ROOT_SPAN:
                        ...
                        addAttributeToRunAction(
                                run, spanAttribute.getAttributeKey(), spanAttribute.getConvertedValue());
                        break;
                    case CURRENT_SPAN:
                        break;
                    default:
                        throw new IllegalArgumentException(
                                "Unsupported target span '" + spanAttribute.getTarget() + "'. ");
                }
            });
            getContext()
                    .newBodyInvoker()
    ...
    private void addAttributeToRunAction(Actionable actionable, AttributeKey attributeKey, Object convertedValue) {
        OpenTelemetryAttributesAction openTelemetryAttributesAction =
                actionable.getAction(OpenTelemetryAttributesAction.class);
    ...
    private OpenTelemetryAttributesAction mergeAttributes(
            StepContext context, List<SpanAttribute> spanAttributes, String currentSpanId)
        ...
        spanAttributes.forEach(spanAttribute -> resultingAttributes
                .getAttributes()
                .put(spanAttribute.getAttributeKey(), spanAttribute.getConvertedValue()));
